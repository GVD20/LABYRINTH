Object.assign(Game, {
    ...
    generate() {
        ...
        编写要求：
            1. 【谜面构建】：构建一个不寻常、引人入胜的场景，激发好奇心。以"发生了什么"或"为什么？"结尾。谜面应简洁，避免信息过载。
            2. 【逻辑自洽】：真相必须能够完美解释谜面中所有的反常点。如果谜面包含误导（如幻觉、谎言、视角局限），真相必须明确解释其成因。
            3. 【核心诡计】：诡计应隐含在谜面线索中，避免过度依赖罕见巧合或冷门专业知识。
            4. 【标题规范】：标题应具备悬疑感，但严禁剧透或暗示核心诡计。
            5. 【难度设置】：当前难度为"${this.state.diff}"。${diffPrompt}
        格式要求：
            1. 【要点提取】：提取出 ${kpCount} 个"谜底要点"（Key Points）。每个要点应是独立的、可验证的事实片段，涵盖谜底的关键因果。
            2. 【Emoji】：选择一个最符合谜题氛围的 Emoji。
            3. 【JSON输出】：严格返回JSON：{"emoji":"","title":"","puzzle":"","answer":"","key_points":[]}。`;
